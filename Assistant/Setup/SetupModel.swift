...
final class SetupModel {
    ...
    private func downloadVoice() async {
        voice = .working("The voice engine, 327 MB, once. Keeps downloading if you leave the app.", 0)
        do {
            try await NaturalVoice.shared.load { fraction in
                Task { @MainActor [weak self] in
                    self?.voice = .working("The voice engine, 327 MB, once. Keeps downloading if you leave the app.", fraction)
                }
            }
            voice = .done
        } catch {
